<template>
  <div class="app-container">
    ...
    <div class="input_top">
      <Input v-model="search" placeholder="搜索姓名或学号" style="width: auto;" @keyup.enter.native="onSearch" />
        ...
        <el-row>
          <el-col :span="12">
            <FormItem label="学号" required>
              <Input v-model="currentData.xh" />
            </FormItem>
          </el-col>
          <el-col :span="12">
            <FormItem label="姓名" required>
              <Input v-model="currentData.studentName" />
            </FormItem>
          </el-col>
        </el-row>
        <el-row>
    ...
    <Drawer
      ...
      width="720"
    >
      <Form :label-width="100" :model="addData" label-position="right"  label-colon=":">
        <el-row>
          <el-col :span="12">
            <FormItem label="学号" required>
              <Input v-model="addData.xh" />
            </FormItem>
          </el-col>
          <el-col :span="12">
            <FormItem label="姓名" required>
              <Input v-model="addData.studentName" />
            </FormItem>
          </el-col>
        </el-row>
        <el-row>
    ...
    >导入excel
      <div>表中必要字段：
        <el-link type="success">学号</el-link>,
        <el-link type="success">姓名</el-link>,
        <el-link type="success">班级</el-link>,
        <el-link type="success">上课院系</el-link>,
        <el-link type="success">专业名称</el-link>,
        <el-link type="success">在校状态</el-link>
      </div>
...
<script>
  ...
  methods: {
    ...
    async fileload() {
      ...
      await importStudentInfoByExcel(param).then(res => {
    ...
    async deleteAll() {
      ...
      await deleteStudentInfoAll().then(res => {
    ...
    async addBook() {
      await addStudentInfo(this.addData).then(res => {
    ...
    async drawerCommit() {
      await putStudentInfo(this.currentData).then(res => {
        clearObject(this.currentData)
        this.drawer = false
    ...
    async deleteObj() {
      ...
      await this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
        ...
        // 确定删除
        deleteStudentInfo(this.currentData.uuid).then(res => {
          // 清除选择的数据
          clearObject(this.currentData)
          this.$Message.success('删除成功。')
        })
    ...
    fetchData() {
      ...
      getStudentInfoList(this.page.current, this.page.size, this.search).then(response => {
